...
from constants import SP_TRAN_TransactionCollectingPartnerDetail_CONFIGURATION
...
dag = DAG(
    '1.7.0.trans_TransactionCollectingPartnerDetailsdata',
    default_args=default_args,
    description='Transfer TransactionCollectingPartnerDetails data',
...
def get_last_sync_date():
    ...
    dest_conn.execute("""
        ...
        WHERE sync_enable = true AND table_name = 'TransactionService_TransactionCollectingPartnerDetail'
    """)
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_TransactionCollectingPartnerDetail_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_TransactionCollectingPartnerDetail_CONFIGURATION}")
        ...
        WHERE table_name = 'TransactionService_TransactionCollectingPartnerDetail'
        ...
        for row in rows:
                ...
                existing_id_query = 'SELECT "TransactionId" FROM "TransactionService_TransactionCollectingPartnerDetail" WHERE "TransactionId" = %s'
                dest_cursor.execute(existing_id_query, (row[0],))
                existing_id = dest_cursor.fetchone()

                if existing_id:
                    # Delete existing row before inserting new data
                    delete_query = 'DELETE FROM "TransactionService_TransactionCollectingPartnerDetail" WHERE "TransactionId" = %s'
                    logging.info(f"Deleting existing row for userid: {row[0]}")
                    dest_cursor.execute(delete_query, (row[0],))

                # Insert new row
                insert_query = f"""
                    INSERT INTO "TransactionService_TransactionCollectingPartnerDetail" ({column_str})
