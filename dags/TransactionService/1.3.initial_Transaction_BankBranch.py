...
dag = DAG(
    ...
    max_active_runs=1,
    concurrency=1,
    tags=['Initial Data Load', 'MIRS4_BankBranch']
)


def get_BankBranch_columns(dest_cur):
    dest_cur.execute("""
        SELECT column_name
        FROM report_table_list
        WHERE table_name = 'TransactionService_BankBranch'
...
def transfer_data():
    try:
        ...
        with source_conn.cursor() as source_cur, dest_conn.cursor() as dest_cur:
            columns = get_BankBranch_columns(dest_cur)
            column_str = ', '.join(columns)

            # Truncate the table only once before starting the batch inserts
            truncate_query = 'TRUNCATE TABLE report.public."TransactionService_BankBranch"'
            ...
            while True:
                query = f"""
                    EXEC dbo.usp_BankBranch_pipeline @Offset={offset}, @FetchNext={batch_size}
                """
                ...
                dest_cur.copy_expert(
                    f"""
                    COPY report.public."TransactionService_BankBranch" ({column_str})
                    FROM STDIN WITH (FORMAT CSV, DELIMITER '\t')
                    """, buffer)
                dest_conn.commit()
                logging.info(f"Batchs inserted into BankBranch table. Rows inserted: {len(rows)}")
...
transfer_data_task = PythonOperator(
    task_id='1.03.initial_BankBranch',
