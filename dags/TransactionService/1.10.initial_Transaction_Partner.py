...
from constants import SP_TRAN_Partner_CONFIGURATION
...
dag = DAG(
    '1.10.0.trans_Partner',
    default_args=default_args,
    description='Transfer Partner data',
...
def get_last_sync_date():
    ...
    dest_conn.execute("""
        ...
        WHERE sync_enable = true AND table_name = 'TransactionService_Partner'
    """)
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_Partner_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_Partner_CONFIGURATION}")
        ...
        WHERE table_name = 'TransactionService_Partner'
