...
from constants import SP_TRAN_TransactionPayingPartnerDetail_CONFIGURATION
...
dag = DAG(
    '1.8.0.trans_TransactionPayingPartnerDetail',
    default_args=default_args,
    description='Transfer TransactionPayingPartnerDetail data',
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_TransactionPayingPartnerDetail_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_TransactionCollectingPartnerDetail_CONFIGURATION}")
        ...
        WHERE table_name = 'TransactionService_TransactionPayingPartnerDetail'
        ...
        for row in rows:
                ...
                existing_id_query = 'SELECT "TransactionId" FROM "TransactionService_TransactionPayingPartnerDetail" WHERE "TransactionId" = %s'
                dest_cursor.execute(existing_id_query, (row[0],))
                existing_id = dest_cursor.fetchone()

                if existing_id:
                    # Delete existing row before inserting new data
                    delete_query = 'DELETE FROM "TransactionService_TransactionPayingPartnerDetail" WHERE "TransactionId" = %s'
                    logging.info(f"Deleting existing row for userid: {row[0]}")
                    dest_cursor.execute(delete_query, (row[0],))

                # Insert new row
                insert_query = f"""
                    INSERT INTO "TransactionService_TransactionPayingPartnerDetail" ({column_str})
