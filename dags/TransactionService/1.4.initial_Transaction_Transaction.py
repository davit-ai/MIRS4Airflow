...
from constants import SP_TRAN_Transaction_CONFIGURATION
...
dag = DAG(
    '1.4.0.trans_Transactionsdata',
    default_args=default_args,
    description='Transfer Transactions data',
...
def get_last_sync_date():
    ...
    dest_conn.execute("""
        ...
        WHERE sync_enable = true AND table_name = 'TransactionService_Transaction'
    """)
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_Transaction_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_Transaction_CONFIGURATION}")
        ...
        for row in rows:
                ...
                existing_id_query = 'SELECT "ReferenceNo" FROM "TransactionService_Transaction" WHERE "ReferenceNo" = %s'
                dest_cursor.execute(existing_id_query, (row[2],))
                existing_id = dest_cursor.fetchone()

                if existing_id:
                    # Delete existing row before inserting new data
                    delete_query = 'DELETE FROM "TransactionService_Transaction" WHERE "ReferenceNo" = %s'
                    logging.info(f"Deleting existing row for userid: {row[0]}")
                    dest_cursor.execute(delete_query, (row[2],))

                # Insert new row
                insert_query = f"""
                    INSERT INTO "TransactionService_Transaction" ({column_str})
