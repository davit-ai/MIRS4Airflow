...
dag = DAG(
    ...
    max_active_runs=1,
    concurrency=1,
    tags=['Initial Data Load', 'MIRS4_PayingMode']
)


def get_PayingMode_columns(dest_cur):
    dest_cur.execute("""
        SELECT column_name
...
def transfer_data():
    try:
        ...
        with source_conn.cursor() as source_cur, dest_conn.cursor() as dest_cur:
            columns = get_PayingMode_columns(dest_cur)
            column_str = ', '.join(columns)

            # Truncate the table only once before starting the batch inserts
            truncate_query = 'TRUNCATE TABLE report.public."TransactionService_PayingMode"'
            ...
            while True:
                query = f"""
                    EXEC dbo.usp_PayingMode_pipeline @Offset={offset}, @FetchNext={batch_size}
                """
                ...
                dest_cur.copy_expert(
                    f"""
                    COPY report.public."TransactionService_PayingMode" ({column_str})
                    FROM STDIN WITH (FORMAT CSV, DELIMITER '\t')
                    """, buffer)
                dest_conn.commit()
                logging.info(f"Batchs inserted into PayingMode table. Rows inserted: {len(rows)}")
...
transfer_data_task = PythonOperator(
    task_id='1.05.initial_PayingMode',
