...
from constants import SP_TRAN_Reference_CONFIGURATION
...
dag = DAG(
    '1.5.0.trans_Referencesdata',
    default_args=default_args,
    description='Transfer References data',
...
def get_last_sync_date():
    ...
    dest_conn.execute("""
        ...
        WHERE sync_enable = true AND table_name = 'TransactionService_Reference'
    """)
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_Reference_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_Reference_CONFIGURATION}")
        ...
        WHERE table_name = 'TransactionService_Reference'
