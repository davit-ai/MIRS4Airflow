...
from constants import SP_TRAN_COUNTRYDATA_CONFIGURATION
...
dag = DAG(
    '1.0.0.trans_countrydata',
    default_args=default_args,
    description='Transfer Country data',
...
def get_last_sync_date():
    ...
    dest_conn.execute("""
        ...
        WHERE sync_enable = true AND table_name = 'TransactionService_Country'
    """)
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_COUNTRYDATA_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_COUNTRYDATA_CONFIGURATION}")
        ...
        WHERE table_name = 'TransactionService_Country'
