...
from constants import SP_TRAN_TransactionSenderDetail_CONFIGURATION
...
dag = DAG(
    '1.3.0.trans_TransactionSenderDetailsdata',
    default_args=default_args,
    description='Transfer TransactionSenderDetails data',
...
def get_last_sync_date():
    ...
    dest_conn.execute("""
        ...
        WHERE sync_enable = true AND table_name = 'TransactionService_TransactionSenderDetails'
    """)
...
def transfer_data():
    try:
        ...
        offset = 0
        # Fetch data
        source_cursor.execute(f"EXEC {SP_TRAN_TransactionSenderDetail_CONFIGURATION} @Offset = {offset}, @FetchNext = {batch_size}")
       ## source_cursor.execute(f"EXEC {SP_TRAN_TransactionSenderDetail_CONFIGURATION}")
        ...
        WHERE table_name = 'TransactionService_TransactionSenderDetail'
        ...
        for row in rows:
                ...
                existing_id_query = 'SELECT "TransactionId" FROM "TransactionService_TransactionSenderDetail" WHERE "TransactionId" = %s'
                dest_cursor.execute(existing_id_query, (row[0],))
                existing_id = dest_cursor.fetchone()

                if existing_id:
                    # Delete existing row before inserting new data
                    delete_query = 'DELETE FROM "TransactionService_TransactionSenderDetail" WHERE "TransactionId" = %s'
                    logging.info(f"Deleting existing row for userid: {row[0]}")
                    dest_cursor.execute(delete_query, (row[0],))

                # Insert new row
                insert_query = f"""
                    INSERT INTO "TransactionService_TransactionSenderDetail" ({column_str})
